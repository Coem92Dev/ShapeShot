extends CharacterBody2D

const SPEED = 100
@export var live : int = 250

@onready var global : GlobalVars = get_node("/root/Global")
@onready var money = preload("res://prefabs/Money/money.tscn")
@onready var laser = preload("res://prefabs/Colleteble/Collecteble.tscn")
@onready var extra = preload("res://prefabs/Colleteble tri/Collecteble_tri.tscn")
@onready var emitter : CPUParticles2D = $CPUParticles2D
@onready var gun1 = $Turret/spr/gun
@onready var gun2 = $Turret2/spr/gun
@onready var turret1 = $Turret
@onready var turret2 = $Turret2

var lock = false
var target : Vector2
func _ready():
	getnewTarget()
	
func _physics_process(delta):
	if(global.player == null):
		return
	
	
	if(global_position.distance_to(global.player.global_position) < 300):

		turret1.look_at(global.player.global_position)
		turret2.look_at(global.player.global_position)
		gun1.spb = 0.5
		gun1.shoot()
		gun2.spb = 0.5
		gun2.shoot()
	if(global_position.distance_to(target) < 5):
		getnewTarget()
	var v = target - global_position
	var a = v.angle()
	global_rotation = lerp_angle(global_rotation,a,10*delta)
		
	velocity = transform.x * SPEED
	emitter.emitting = true
	emitter.lifetime = 1
		
	move_and_slide()


func hurt(hitAmount : int):
	live -= hitAmount
	if(!lock):
		if(live <= 0):
			lock=true
			if(randf() > 0.5):
				var l = laser.instantiate()
				l.global_position = global_position
				global.world.add_child.call_deferred(l)
			else:
				var e = extra.instantiate()
				e.global_position = global_position
				global.world.add_child.call_deferred(e)
			
			var moneyspawn = randi_range(45,50)
			for i in moneyspawn:
				var m = money.instantiate()
				if(randf() > 0.75):
					m.value = 5
				else:
					m.value = 1
				m.global_position = global_position + Vector2(randi_range(-15,15),randi_range(-15,15))
				m.rotation_degrees = randf_range(0,90)
				global.world.add_child.call_deferred(m)

			global.killCounter += 1
			queue_free()


func getnewTarget():
		if(global.player != null):
			target = global.player.global_position+Vector2(250,0).rotated(deg_to_rad(randf()*360))


func _on_timer_timeout():
	getnewTarget()
	pass # Replace with function body.
